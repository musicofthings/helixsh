...
def test_audit_verify_detects_missing_hash(tmp_path, capsys):
    ...
    try:
        ...
        assert payload["mismatched_hash"] == 0
    finally:
        cli.AUDIT_FILE = old


def test_audit_verify_detects_mismatched_hash(tmp_path, capsys):
    old = cli.AUDIT_FILE
    cli.AUDIT_FILE = tmp_path / "audit.jsonl"
    cli.AUDIT_FILE.write_text(
        '{"timestamp":"x","command":"nextflow run x","strict":false,"mode":"run","role":"analyst","execution_hash":"bad","provenance_params":{"a":1}}\n',
        encoding="utf-8",
    )
    try:
        rc = cli.main(["audit-verify"])
        assert rc == 2
        payload = json.loads(capsys.readouterr().out)
        assert payload["mismatched_hash"] == 1
    finally:
        cli.AUDIT_FILE = old
